-- Fixed Database Schema Updates for Industries and Macro Economy Analysis
...
-- 2. UPDATE SOURCES TABLE to include source_type (safe to run multiple times)
ALTER TABLE sources ADD COLUMN IF NOT EXISTS source_type text DEFAULT 'company';

-- Add CHECK constraint for source_type if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
